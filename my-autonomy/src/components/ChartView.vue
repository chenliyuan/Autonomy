 <template>
  <div id="divContainer">
    ...
    <van-cell-group title="日喂奶量" v-if="feedList.length>0" >
      ...
      >
      <template #default>
        <div><span style="font-weight:600;font-size:18px;color:blue">{{item.sumvalue}}</span> /{{item.count}}</div>
      </template>
      </van-cell>
    </van-cell-group>
...
export default {
  ...
  created() {
    ...
    for (let content of this.subItem.contents) {
      ...
      if (contentValue > this.max) {//计算出最大值
        this.max = contentValue;
      }
      if (contentValue< this.min) {//计算最小值
        this.min = contentValue;
      }
      if (contentValue != 0) {//计算有效个数
        this.validnum += 1;
      }
    }
